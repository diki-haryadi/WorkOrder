...
const generateNumber = () => `INV-${Date.now().toString().slice(-6)}`;
    ...
    return (
      <div className="flex flex-col h-full">
        <div className="flex items-center gap-3 px-4 py-4 border-b border-slate-100 bg-white sticky top-0 z-10">
          ...
          <h2 className="text-base font-semibold text-slate-800 flex-1">{selected ? 'Edit Invoice' : 'Invoice Baru'}</h2>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <FormField label="Status">
              <Select value={form.status ?? 'draft'} onChange={e => setForm(f => ({ ...f, status: e.target.value as InvoiceStatus }))}>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-sm font-medium text-slate-700">Item Tagihan</label>
            ...
            <div className="space-y-2">
              {lineItems.map((item, idx) => (
                <div key={item.id} className="bg-slate-50 rounded-xl p-3 space-y-2">
                  ...
                  <Input placeholder="Deskripsi item" value={item.description} onChange={e => updateLineItem(item.id, 'description', e.target.value)} />
                  <div className="grid grid-cols-3 gap-2">
                    <div>
                      <p className="text-xs text-slate-400 mb-1">Qty</p>
                      <Input type="number" min="1" value={item.qty} onChange={e => updateLineItem(item.id, 'qty', Number(e.target.value))} />
                    </div>
                    <div className="col-span-2">
                      <p className="text-xs text-slate-400 mb-1">Harga Satuan</p>
                      <Input type="number" min="0" value={item.unit_price} onChange={e => updateLineItem(item.id, 'unit_price', Number(e.target.value))} />
                    </div>
                  </div>
          ...
          <div className="bg-slate-50 rounded-xl p-3 space-y-2">
            ...
            <div className="flex items-center justify-between text-sm gap-3">
              ...
              <div className="flex items-center gap-2 ml-auto">
                <Select value={form.tax_rate ?? 11} className="w-20 py-1" onChange={e => {
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-4 space-y-2">
            <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Item Tagihan</h4>
            ...
            <button key={item.id} onClick={() => { setSelected(item); setView('detail'); }} className={`w-full bg-white rounded-2xl border border-slate-100 border-l-4 ${statusColor[item.status]} p-4 text-left hover:shadow-sm active:scale-[0.99] transition-all`}>
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">{item.invoice_number}</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <p className="text-sm font-bold text-blue-600">{formatIDR(item.total)}</p>
                  {item.due_date && <p className="text-xs text-slate-400">{new Date(item.due_date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}</p>}
                </div>
              </div>
            </button>
          ))
